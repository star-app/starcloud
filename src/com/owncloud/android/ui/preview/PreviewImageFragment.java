...
import com.owncloud.android.ui.activity.TencentLoginActivity;
...
public class PreviewImageFragment extends SherlockFragment implements   FileFragment, 
    ...
    private void registerIWXAPI(){
        api = WXAPIFactory.createWXAPI(this.getActivity(),"wx08a3c88f4f21d8a8");
        api.registerApp("wx08a3c88f4f21d8a8");
    }
    /**
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mImageView.setOnTouchListener(new MulitPointTouchListener (getActivity()));
        return mView;
    }
    
    private int reflectRatio = 4;
    private Bitmap imageAddReflection(Bitmap originalImage){
        ...
        Bitmap reflectionImage = Bitmap.createBitmap(originalImage, 0,
                (reflectRatio-1)*height / reflectRatio, width, height / reflectRatio, matrix, false);

        Bitmap bitmapWithReflection = Bitmap.createBitmap(width,
                (height + height / reflectRatio), Config.ARGB_8888);
    ...
    public void onStart() {
        super.onStart();
        if (mFile != null) {
//            ImageView imag = new TouchImageView(getActivity());
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.action_share_file_to_friend: {
                shareWXImage(SendMessageToWX.Req.WXSceneSession);
                return true;
            }
            case R.id.action_share_file_to_circle: {
                shareWXImage(SendMessageToWX.Req.WXSceneTimeline);
                return true;
            }
            case R.id.action_share_file_to_qqzone:{
                shareQZoneImage();
            }
...
//    	Intent intent = new Intent(this.getActivity(),ShareScreenService.class);
//    	intent.s\
        
//        LandingActivity.shareInit.share(uri);
    }
    private void shareWXImage(int scene) {
        Bitmap bmp = ImageUtil.loadImage(mFile.getStoragePath());        
        
        Bitmap image_100K = ImageUtil.resizeBitmap(bmp, 100*1024);
        if(bmp != image_100K){
            bmp.recycle();
        }
        
        Bitmap image_10k = ImageUtil.resizeBitmap(image_100K, 10*1024);
        
        WXImageObject imgObj = new WXImageObject(image_100K);
        image_100K.recycle();
        
        WXMediaMessage msg = new WXMediaMessage();
        msg.mediaObject = imgObj;
        
        msg.thumbData = ImageUtil.getBitmapArray(image_10k);
        image_10k.recycle();
    ...
    private void shareQZoneImage(){
        Tencent tencent = TencentLoginActivity.getTencent();
        if(tencent == null || !tencent.isSessionValid()){
            Intent intent = new Intent(this.getActivity(),TencentLoginActivity.class);
            startActivity(intent);      
            if(TencentLoginActivity.getTencent() == null){
                //È¡ÏûµÇÂ½
                return;
            }
        }
        tencent = TencentLoginActivity.getTencent();
        //tencent.request(arg0, arg1, arg2);
     }
    ...
    private class BitmapLoader extends AsyncTask<String, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(String... params) {
            ...
            try {
                ...
                result = imageAddReflection(result);
                if (result == null) {
